...
def verify_contractor_pin(
    ...
    if c.is_active == 0:
        return {"success": False, "reason": "blocked", "message": "आपकी authority suspend की गई है। कृपया Officer से संपर्क करें।"}
    return {"success": True}
...
@router.put("/{contractor_id}/authority")
def toggle_contractor_authority(contractor_id: int, is_active: int, db: Session = Depends(get_db)):
    """Officer activates (is_active=1) or deactivates (is_active=0) a contractor."""
    c = db.query(Contractor).get(contractor_id)
    if not c:
        return {"message": "Contractor not found"}
    c.is_active = is_active
    db.commit()
    action = "Activated" if is_active == 1 else "Deactivated"
    return {"message": f"Contractor {action} successfully", "is_active": is_active}

...
import SchemesChatbot from "./components/SchemesChatbot";
...
export default function App() {
  ...
  return (
    <div className="relative min-h-screen font-sans">
      ...
      {/* 🤖 Floating Yojana Saathi Chatbot Button */}
      <motion.button
        ...
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 1 }}
        className="fixed bottom-8 right-8 z-40 flex items-center gap-3 bg-gradient-to-r from-emerald-600 to-teal-600 text-white px-6 py-4 rounded-[20px] shadow-2xl shadow-emerald-500/40 font-black text-sm uppercase tracking-widest hover:from-emerald-500 hover:to-teal-500 transition-all border border-white/20"
      >
        <span className="text-2xl">🤖</span>
        <div className="leading-none">
          <div>Yojana Saathi</div>
          <div className="text-[10px] font-medium opacity-80 mt-0.5">सरकारी योजना सहायक</div>
        </div>
      ...
      <AnimatePresence>
        {showChatbot && (
          <SchemesChatbot onClose={() => setShowChatbot(false)} />
        )}
      </AnimatePresence>
    </div>
  );
